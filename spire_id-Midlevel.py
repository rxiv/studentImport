...
filepath= fd.askopenfilename(title='SPIRE Mid Student Data', initialdir='.', filetypes=(('CSV files','*.csv'),('All files','*.*')))
if filepath == '': exit()

sheet = pd.read_csv(filepath, keep_default_na=True)
...
datafile['Score'] = datafile.iloc[:,12:40].mean(axis=1, numeric_only=True)

now = datetime.now()
dt = now.strftime("%Y%m%d-%M%S")
datafile.to_excel('spire-mid-merged-'+dt+'.xlsx', index=False)
